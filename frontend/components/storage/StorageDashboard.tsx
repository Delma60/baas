  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-3 py-3 border-b border-[--border]">
        ...
        <CreateBucketDialog projectId={projectId} onCreated={(name) => { onCreated(name); onClose?.(); }} />
      </div>
        ...
        <DialogContent className="max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Buckets are containers for your files. Use lowercase letters, numbers, and hyphens.
            </DialogDescription>
          </DialogHeader>
...
function UploadZone({
  ...
  const uploadFile = React.useCallback(async (item: UploadItem) => {
    ...
    try {
      ...
      onUploaded({ key, size: item.file.size, last_modified: new Date().toISOString(), etag: "" });
    } catch (err) {
    ...
    <div className="mx-4 sm:mx-6 my-4 rounded-xl border-2 border-dashed border-[--border2] bg-[--background] overflow-hidden">
      ...
      >
        ...
        <div className="text-center px-4">
          ...
          <p className="text-xs text-[--text-muted] mt-1">Files are uploaded securely via direct transfer</p>
        </div>
            ...
            <div key={item.id} className="flex items-center gap-3 px-4 py-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[10px] text-[--text-muted] whitespace-nowrap shrink-0">{formatBytes(item.file.size)}</span>
                </div>
              </div>
              ...
              {item.status === "error" && <AlertCircle className="h-4 w-4 text-[--danger] shrink-0" title={item.error} />}
            </div>
          ))}
        </div>
      )}
...
function FileRow({
  ...
  return (
    <div className={cn(
      ...
      <div className="w-7 shrink-0" onClick={e => { e.stopPropagation(); onToggleSelect(); }}>
        <input
          type="checkbox"
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-[--text-muted] truncate font-mono hidden sm:block">{file.key}</p>
      </div>
      <div className="hidden md:block text-xs text-[--text-muted] tabular-nums shrink-0 w-16 text-right">{formatBytes(file.size)}</div>
      <div className="hidden lg:block text-[11.5px] text-[--text-muted] shrink-0 w-28 text-right">{formatDateShort(file.last_modified)}</div>
      ...
      <div className="absolute top-2 right-2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-36">
            <DropdownMenuItem className="text-xs gap-2 cursor-pointer" onClick={() => onCopyUrl(file.key)}><Copy className="h-3.5 w-3.5" /> Copy link</DropdownMenuItem>
...
function StatsBar({ files, activeBucket }: { files: StorageFile[]; activeBucket: string }) {
  const totalSize = files.reduce((acc, f) => acc + f.size, 0);
  const imageCount = files.filter(f => getFileType(f.key) === "image").length;

  const stats = [
    { label: "Files", value: String(files.length), icon: File, bg: "bg-[--info-bg]", color: "text-[--info-text]" },
    { label: "Total size", value: formatBytes(totalSize), icon: HardDrive, bg: "bg-amber-500/10", color: "text-amber-500" },
    { label: "Images", value: String(imageCount), icon: FileImage, bg: "bg-[--success-bg]", color: "text-[--success-text]" },
  ];
        ...
        return (
          <div key={s.label} className="rounded-xl border border-[--border] bg-[--background] p-3 sm:p-4">
            <div className="flex items-center gap-2 mb-2">
              <div className={cn("flex h-6 w-6 sm:h-7 sm:w-7 items-center justify-center rounded-[8px]", s.bg)}>
                <Icon className={cn("h-3 w-3 sm:h-3.5 sm:w-3.5", s.color)} />
              </div>
              <span className="text-[10px] sm:text-xs text-[--text-secondary] hidden sm:block">{s.label}</span>
            </div>
            <p className="text-lg sm:text-2xl font-medium text-[--text-primary] leading-none">{s.value}</p>
            <p className="text-[10px] text-[--text-muted] mt-1 sm:hidden">{s.label}</p>
          </div>
        );
      })}
    </div>
            ...
            <div className="border-t border-[--border] p-3">
              <div className="rounded-lg bg-[--surface] border border-[--border] p-3 space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-medium text-[--text-primary]">{formatBytes(files.reduce((a, f) => a + f.size, 0))}</span>
                </div>
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-6 py-3 border-b border-[--border] bg-[--background] shrink-0">
            ...
            <div className="flex items-center gap-1 text-[13px] text-[--text-muted] min-w-0">
              <span className="font-mono font-medium text-[--text-primary] truncate max-w-[80px] sm:max-w-none">{activeBucket}</span>
              {prefix && <><ChevronRight className="h-3.5 w-3.5 shrink-0" /><span className="font-mono truncate">{prefix}</span></>}
            </div>
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={handleRefresh} disabled={isLoadingFiles}>
              <RefreshCw className={cn("h-3.5 w-3.5", isLoadingFiles && "animate-spin")} />
            </Button>

            <Button size="sm" className="h-8 gap-1.5 text-xs bg-brand hover:bg-brand-hover text-white border-0 shrink-0" onClick={() => setShowUpload(s => !s)}>
            ...
            {filteredFiles.length === 0 ? (
              ...
              <div>
                ...
                <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5 border-b border-[--border] bg-[--surface] sticky top-0">
                  ...
                  <span className="hidden md:block text-[11px] font-semibold uppercase tracking-wider text-[--text-muted] w-16 text-right">Size</span>
                  <span className="hidden lg:block text-[11px] font-semibold uppercase tracking-wider text-[--text-muted] w-28 text-right">Modified</span>
                  <div className="w-16" />
                </div>
