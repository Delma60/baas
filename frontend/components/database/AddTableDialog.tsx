...
import {
  ...
  Hash, Type, ToggleLeft, Calendar, Braces, Key, Link2,
} from "lucide-react";
...
const TYPE_COLORS: Record<string, string> = {
  text: "text-amber-500", integer: "text-blue-500", float: "text-blue-400",
  boolean: "text-emerald-500", timestamp: "text-violet-500",
  jsonb: "text-orange-500", uuid: "text-pink-500",
};
...
function makeCol(): ColumnDef {
  return {
    ...
    foreignKey: { enabled: false, table: "", column: "id", onDelete: "NO ACTION" },
  };
}

export function AddTableDialog({ open, onClose, projectId, dbSchema, existingTables, onCreated }: Props) {
  const [tableName, setTableName] = React.useState("");
  const [tableNameError, setTableNameError] = React.useState("");
  const [enableRealtime, setEnableRealtime] = React.useState(false);
  const [columns, setColumns] = React.useState<ColumnDef[]>([{ ...makeCol(), name: "title", nullable: false }]);
  const [isLoading, setIsLoading] = React.useState(false);
  const [error, setError] = React.useState("");

  React.useEffect(() => {
    if (open) {
      setTableName(""); setTableNameError(""); setError("");
  ...
  const handleSubmit = async () => {
    ...
    for (const col of columns) {
      if (!col.name.trim()) { setError("All columns must have a name."); return; }
      if (col.foreignKey.enabled && !col.foreignKey.table) {
        setError(`Column "${col.name}" has a foreign key enabled but no table selected.`);
        return;
      }
    }
        ...
        body: JSON.stringify({
          ...
          columns: columns.map(c => ({
            ...
            foreign_key: c.foreignKey.enabled
              ? { table: c.foreignKey.table, column: c.foreignKey.column, on_delete: c.foreignKey.onDelete }
              : null,
          })),
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-5 py-2 pr-1">
          ...
          <div className="space-y-1.5">
            ...
            {tableNameError && <p className="text-[11px] text-destructive">{tableNameError}</p>}
          </div>
          ...
          <div className="rounded-lg border border-dashed border-border bg-muted/30 p-3 space-y-1.5">
            ...
            {[
              ...
              <div key={c.name} className="flex items-center gap-3 text-[12px]">
                ...
                <Badge variant="secondary" className="text-[10px] h-4 px-1.5 font-mono">{c.type}</Badge>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-3">
              {columns.map(col => {
                ...
                return (
                  <div key={col.id} className="rounded-lg border border-border bg-muted/10 p-3 space-y-2">
                    ...
                    <div className="grid grid-cols-[1fr_140px_60px_32px] gap-2 items-center">
                      ...
                      />
                      <Select value={col.type} onValueChange={v => updateCol(col.id, { type: v })}>
                      ...
                      <div className="flex items-center justify-center gap-1">
                        ...
                        <Switch checked={col.nullable} onCheckedChange={v => updateCol(col.id, { nullable: v })} className="scale-75" />
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => updateFk(col.id, { enabled: !col.foreignKey.enabled })}
                        className={cn(
                    ...
                    {col.foreignKey.enabled && (
                      <div className="grid grid-cols-3 gap-2 pl-2 border-l-2 border-blue-200 dark:border-blue-800">
                        <div className="space-y-1">
                          <p className="text-[10px] text-muted-foreground font-medium">References table</p>
                          <Select
                            value={col.foreignKey.table}
                            onValueChange={v => updateFk(col.id, { table: v })}
                          >
                            <SelectTrigger className="h-8 text-xs">
                              <SelectValue placeholder="Select table…" />
                            </SelectTrigger>
                            <SelectContent>
                              {existingTables?.filter(t => t !== cleanedName && !t.startsWith("_")).map(t => (
                                <SelectItem key={t} value={t} className="text-xs font-mono">{t}</SelectItem>
                              ))}
                              {existingTables?.filter(t => !t.startsWith("_")).length === 0 && (
                        ...
                        <div className="space-y-1">
                          <p className="text-[10px] text-muted-foreground font-medium">References column</p>
                          <Input
                            value={col.foreignKey.column}
                            onChange={e => updateFk(col.id, { column: e.target.value })}
                            className="h-8 text-xs font-mono"
                            placeholder="id"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          <p className="text-[10px] text-muted-foreground font-medium">On delete</p>
                          <Select
                            value={col.foreignKey.onDelete}
                            onValueChange={v => updateFk(col.id, { onDelete: v })}
                          >
                            <SelectTrigger className="h-8 text-xs">
                              <SelectValue />
                            </SelectTrigger>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border bg-muted/20 px-3.5 py-3">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground">Broadcast INSERT / UPDATE / DELETE via pg_notify</p>
            </div>
