...
const NAV_ITEMS = [
  { label: "Overview",    href: "overview",  icon: LayoutDashboard, comingSoon: false },
...
function NavItem({
  ...
  if (item.comingSoon) {
    return (
      <div
        className={cn(
          "flex items-center gap-2.5 h-8 px-2.5 rounded-lg text-sm font-medium",
          "text-text-muted cursor-not-allowed opacity-60 select-none",
        )}
        title="Coming soon"
      >
        <Icon className="h-4 w-4 shrink-0 text-sidebar-icon" />
        <span className="flex-1">{item.label}</span>
        <span className="rounded-full bg-brand/10 px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider text-brand leading-none">
          Soon
        </span>
      </div>
    );
        ...
        {projects?.map((p) => (
          <DropdownMenuItem key={p.id}>
            <Link
              ...
              className="flex items-center gap-2.5 px-2 py-1.5 rounded-md cursor-pointer w-full"
...
function PlanBadge({ plan }: { plan: "free" | "starter" | "pro" }) {
  ...
  return (
    <span className={cn("rounded-full border px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider", styles[plan])}>
      {plan}
    </span>
  );
}
...
function SubscriptionCard({ userId }: { userId: string }) {
  ...
  const planInfo = {
    ...
    pro: {
      ...
      cta: "Manage billing",
      href: `/u/${userId}/billing`,
      ctaStyle: "bg-surface hover:bg-surface-hover text-text-primary border border-border",
      progress: 71,
    },
  };
    ...
    <div className="rounded-xl border border-border bg-background p-3">
      ...
      <div className="mb-2.5">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[10px] font-medium text-text-secondary">{info.progress}%</span>
        </div>
        <div className="h-1 w-full rounded-full bg-surface">
          <div
            className={cn(
              "h-full rounded-full transition-all",
              info.progress > 80 ? "bg-danger" : info.progress > 60 ? "bg-warning" : "bg-brand",
            )}
      ...
      <div className="p-2.5 space-y-2 border-t border-border">
        ...
        >
          <Settings
            className={cn(
              "h-4 w-4 shrink-0",
              isActive("settings") ? "text-sidebar-active-text" : "text-sidebar-icon",
            )}
          />
          Settings
        </Link>
...
export function Sidebar({
  ...
  const sharedProps = { userId, projectId, currentProject, projects };

  return (
    <>
      {/* ── Desktop sidebar — hidden on mobile ── */}
      <aside className="hidden md:flex h-full w-[220px] shrink-0 flex-col border-r border-border bg-sidebar-bg">
        {/* Brand header */}
        <div className="flex h-14 items-center gap-2 border-b border-border px-3">
          ...
          >
            ...
            <span className="text-xs font-semibold text-text-secondary">YourBaaS</span>
          </Link>
        </div>
      ...
      <Sheet open={mobileOpen} onOpenChange={onMobileOpenChange}>
        <SheetContent side="left" className="w-[260px] p-0 bg-sidebar-bg border-r border-border">
          <SheetHeader className="flex h-14 flex-row items-center justify-between border-b border-border px-3">
          ...
          <div className="h-[calc(100%-3.5rem)] overflow-hidden">
            <SidebarContent {...sharedProps} onNavigate={() => onMobileOpenChange?.(false)} />
          </div>
        </SheetContent>
      </Sheet>
    </>
