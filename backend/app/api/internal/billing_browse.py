...
async def billing_overview(project_id: str, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
...
async def get_subscription(project_id: str, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    org_id = await _get_org_id_from_project(db, project_id)
...
async def initiate_checkout(
    ...
    # Format: baas_proj_{project_id}_{plan}_{random8}
    ...
    payload = {
        ...
        "meta": {"project_id": project_id, "plan": body.plan},
    ...
    await db.execute(
        ...
        """),
        {"id": str(uuid.uuid4()), "org_id": org_id, "tx_ref": tx_ref, "amount": amount, "currency": body.currency},
    )
    await db.commit()
...
async def verify_checkout(
    ...
    # Split: ['baas', 'proj', project_id, plan, random8]
    parts = body.tx_ref.split("_")
    new_plan = parts[3] if len(parts) >= 5 else "starter"
    ...
    await db.execute(
        ...
        """),
        {
            "id": str(uuid.uuid4()), "org_id": org_id, "plan": new_plan,
            "tx_ref": body.tx_ref, "tx_id": str(body.transaction_id),
            "period_start": now, "period_end": period_end,
            "amount_ngn": amount_ngn, "amount_usd": amount_usd,
        },
    )
    ...
    await db.execute(
        ...
        """),
        {
            ...
            "tx_id": str(body.transaction_id), "tx_ref": body.tx_ref,
            "amount": float(tx.get("amount", 0)), "currency": tx.get("currency", "NGN"),
    ...
    # Bust cached API keys for this project
    try:
...
async def cancel_subscription(project_id: str, db: AsyncSession = Depends(get_db)) -> dict[str, Any]:
    org_id = await _get_org_id_from_project(db, project_id)
    await db.execute(
        text("""
...
async def flutterwave_webhook(
    ...
    if tx_ref.startswith("baas_proj_"):
        ...
        # indices: baas(0) proj(1) {project_id}(2) {plan}(3) {random}(4)
        extracted_project_id = parts[2] if len(parts) >= 5 else None
        if extracted_project_id:
            try:
                org_id = await _get_org_id_from_project(db, extracted_project_id)
            except HTTPException:
                logger.warning("Webhook: project %s not found in tx_ref %s", extracted_project_id, tx_ref)
    ...
    await db.execute(
        ...
        {
            "id": str(uuid.uuid4()), "org_id": org_id,
            "event_type": event, "tx_id": tx_id, "tx_ref": tx_ref,
            "amount": float(data.get("amount", 0)), "currency": data.get("currency", "NGN"),
            "status": status, "payload": json.dumps(payload),
        },
    )
    ...
    if event == "charge.completed" and status == "successful" and org_id:
        ...
        await db.execute(
            ...
            """),
            {
                "id": str(uuid.uuid4()), "org_id": org_id,
                "amount_ngn": amount_ngn, "amount_usd": amount_usd,
                "payment_ref": tx_id, "period_start": now, "period_end": period_end,
            },
        )
